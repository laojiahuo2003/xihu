...
public class PosterBaiduController {
    ...
    public Result GetImage(@org.springframework.web.bind.annotation.RequestBody Map<String, String> params) throws IOException, InterruptedException {
        ...
        Integer taskId = submitText(text);
     ...
     * @throws IOException
     */
    private Integer submitText(String text) throws IOException {

        // 构建请求体 JSON
        JSONObject requestBodyJson = new JSONObject();
        requestBodyJson.put("text", text);
        requestBodyJson.put("resolution", "1024*1024");
        requestBodyJson.put("style", "写实风格");
        // 发送请求
        MediaType mediaType = MediaType.parse("application/json");
        RequestBody body = RequestBody.create(mediaType,requestBodyJson.toString());
        Request request = new Request.Builder()
